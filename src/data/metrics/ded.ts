...
export function computeDED(rows: Row[]): { metrics: LdnStageMetrics; issues: ActionableIssue[] } {
  ...
  // Box 1: Cases with DED — total matters that have a DED set
  const casesWithDed = uniqueMatterCount(withDED);
  ...
  const past90 = uniqueMatterCount(past90Rows.map(d => d.row));

  // Box 4: 180+ Days Past DED
  const past180Rows = past.filter(d => Math.abs(d.days) >= 180);
  const past180 = uniqueMatterCount(past180Rows.map(d => d.row));

  let ragColor: RagColor = 'green';
  if (past180 > 0) ragColor = 'red';
  else if (past90 > 0) ragColor = 'amber';

  const agingValues = parsed.map(d => Math.abs(d.days));

  const cards: MetricCard[] = [
    { label: 'Cases with DED', value: casesWithDed, rag: 'green' },
    { label: 'Avg Days Past DED', value: pastDays.length > 0 ? `${avgPast}d` : '-', rag: avgPast >= 180 ? 'red' : avgPast >= 90 ? 'amber' : 'green' },
    { label: '90+ Days Past', value: past90, rag: past90 === 0 ? 'green' : 'amber' },
    { label: '180+ Days Past', value: past180, rag: past180 === 0 ? 'green' : 'red' },
  ];
