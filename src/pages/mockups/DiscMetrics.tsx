...
import { discMetrics, discKPICategories, discSLARules, discEscalationTriggers, discDepoChecklist } from '../../data/discMetricsData.ts';
import { Badge } from '../../components/ui/badge.tsx';
import { Button } from '../../components/ui/button.tsx';
import { ChevronDown, ChevronRight, AlertTriangle, Target, Clock, TrendingUp, ClipboardCheck, CheckCircle2 } from 'lucide-react';
...
export default function DiscMetrics() {
  // Scorecard state
  const [metricValues, setMetricValues] = useState<Record<string, number>>(() => {
    const init: Record<string, number> = {};
    discMetrics.forEach((m) => {
      init[m.id] = 0;
    });
    return init;
  });
  ...
  // Deposition Readiness Checklist state
  const [depoChecked, setDepoChecked] = useState<Record<string, boolean>>({});
  const toggleDepoCheck = (id: string) => {
    setDepoChecked(prev => ({ ...prev, [id]: !prev[id] }));
  };
  ...
  const sortedSLARules = [...discSLARules].sort((a, b) => {
    ...
    <div className="flex-1 overflow-auto p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-3">
          Discovery — Weekly Scorecard &amp; KPIs
        </h1>
        <MockupNav active="disc-metrics" group="discovery" />
      </div>
        ...
        <section className="mb-12">
          <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Comprehensive KPI definitions across {discKPICategories.length} categories
              </p>
            </div>
            <Button
              variant="outline"
          ...
          <div className="space-y-3">
            {discKPICategories.map((category) => {
        ...
        {/* ─── Deposition Readiness Checklist ──────────────────────────── */}
        <section className="mb-12">
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-1">
              <ClipboardCheck className="h-5 w-5 text-muted-foreground" />
              <h2 className="text-xl font-semibold">Deposition Readiness Checklist</h2>
            </div>
            <p className="text-sm text-muted-foreground">
              Interactive checklist tracking all deposition preparation requirements
            </p>
          </div>

          {(['core', 'exhibits', 'risk-flags'] as const).map((category) => {
            const categoryLabel = category === 'core' ? 'Core Requirements' : category === 'exhibits' ? 'Exhibits' : 'Risk Flags';
            const categoryItems = discDepoChecklist.filter(item => item.category === category);
            const checkedCount = categoryItems.filter(item => depoChecked[item.id]).length;
            const allChecked = checkedCount === categoryItems.length;

            return (
              <div key={category} className="mb-4 rounded-lg border border-border bg-card overflow-hidden">
                ...
                <div className="divide-y divide-border">
                  {categoryItems.map((item) => (
                    <label key={item.id} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-muted/30 transition-colors">
                      <input
                        type="checkbox"
                        checked={!!depoChecked[item.id]}
                        onChange={() => toggleDepoCheck(item.id)}
                        className="h-4 w-4 accent-blue-800"
                      />
                      <span className={cn('text-sm', depoChecked[item.id] ? 'line-through text-muted-foreground' : 'text-foreground')}>
                        {item.label}
                      </span>
                      {depoChecked[item.id] ? (
                        <CheckCircle2 className="ml-auto h-4 w-4 text-green-500" />
                      ) : (
                        <div className="ml-auto h-4 w-4 rounded-full border-2 border-muted-foreground/30" />
                      )}
                    </label>
                  ))}
                </div>
        ...
        <section className="mb-12">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {discEscalationTriggers.map((trigger) => (
              <div
                key={trigger.id}
