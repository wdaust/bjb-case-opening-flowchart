...
import { trialPhases, trialTasks } from '../../data/trialTaskData.ts';
...
const phaseBorderColors: Record<string, string> = {
  'violet-700': 'border-l-violet-700',
  'sky-700': 'border-l-sky-700',
  'fuchsia-700': 'border-l-fuchsia-700',
...
export default function TrialTracker() {
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(
    () => new Set(trialPhases.map((p) => p.id))
  );
  ...
  // Progress
  const completedCount = trialTasks.filter((t) => statuses[t.id] === 'complete').length;
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-3">10-Task Tracker</h1>
        <MockupNav active="trial-tracker" group="trial" />
      </div>
