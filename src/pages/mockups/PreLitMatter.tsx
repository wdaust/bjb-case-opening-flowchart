...
import {
  ...
  preLitTasks,
  PRELIT_PATH_STAGES,
  getPreLitTasksForStage,
} from '../../data/preLitTaskData.ts';

const STORAGE_KEY = 'bjb-prelit-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  preLitTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function PreLitMatter() {
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(preLitPhases.map((p) => p.id)));

  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(taskStatuses)); } catch { /* ignore */ }
  }, [taskStatuses]);

  const completedCount = preLitTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = preLitTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(preLitTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  const stageProgress: PathBarStage[] = useMemo(() => {
    return PRELIT_PATH_STAGES.map((stage) => {
      const tasks = getPreLitTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof preLitTasks>();
  ...
  const visiblePhases = preLitPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(preLitPhases.map((p) => p.id)));
  }, []);
  ...
  const overviewContent = (
    <OverviewTab tasks={preLitTasks} phases={preLitPhases} statuses={taskStatuses} scoringSystems={scoringSystems} scores={scores} />
  );
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {preLitPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = preLitTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{preLitPhases.filter((p) => preLitTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
