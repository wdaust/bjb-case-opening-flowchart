...
const STORAGE_KEY = 'bjb-co-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  CO_CONTACT_TASKS.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function CaseOpeningMatter() {
  // ── State ──────────────────────────────────────────────────────────
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(CO_PHASES.map((p) => p.id)));
  ...
  // ── Derived ────────────────────────────────────────────────────────
  const completedCount = CO_TRACKER_TASKS.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = CO_TRACKER_TASKS.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(CO_TRACKER_TASKS.map((t) => t.assignedTo));
  ...
  const filteredTasks = useMemo(() => {
    return CO_TRACKER_TASKS.filter((t) => {
  ...
  const visiblePhases = CO_PHASES.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(CO_PHASES.map((p) => p.id)));
  }, []);
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {CO_PHASES.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = CO_TRACKER_TASKS.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{CO_PHASES.filter((p) => CO_TRACKER_TASKS.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
