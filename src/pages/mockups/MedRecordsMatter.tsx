...
import {
  ...
  medRecordsTasks,
  MEDREC_PATH_STAGES,
  getMedRecordsTasksForStage,
} from '../../data/medRecordsTaskData.ts';

const STORAGE_KEY = 'bjb-medrec-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  medRecordsTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}

const phaseBorderColors: Record<string, string> = {
  'blue-700': 'border-l-blue-700', 'green-700': 'border-l-green-700', 'violet-700': 'border-l-violet-700',
  'amber-700': 'border-l-amber-700', 'indigo-700': 'border-l-indigo-700', 'rose-700': 'border-l-rose-700',
  'teal-700': 'border-l-teal-700', 'purple-700': 'border-l-purple-700', 'sky-700': 'border-l-sky-700',
  'fuchsia-700': 'border-l-fuchsia-700', 'red-700': 'border-l-red-700',
};
...
export default function MedRecordsMatter() {
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(medRecordsPhases.map((p) => p.id)));

  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(taskStatuses)); } catch { /* ignore */ }
  }, [taskStatuses]);

  const completedCount = medRecordsTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = medRecordsTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(medRecordsTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  const stageProgress: PathBarStage[] = useMemo(() => {
    return MEDREC_PATH_STAGES.map((stage) => {
      const tasks = getMedRecordsTasksForStage(stage);
  ...
  const filteredTasks = useMemo(() => {
    return medRecordsTasks.filter((t) => {
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof medRecordsTasks>();
  ...
  const visiblePhases = medRecordsPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(medRecordsPhases.map((p) => p.id)));
  }, []);
  ...
  const overviewContent = (
    <OverviewTab tasks={medRecordsTasks} phases={medRecordsPhases} statuses={taskStatuses} scoringSystems={scoringSystems} scores={scores} />
  );
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {medRecordsPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = medRecordsTasks.filter((t) => t.phase === phase.id);
      ...
      <Tabs value={activeScoringTab} onValueChange={setActiveScoringTab}>
        ...
        {scoringSystems.map((sys) => (
          <TabsContent key={sys.id} value={sys.id}>
            <ScoringSystemPage system={sys} scores={scores} onScoreChange={handleScoreChange} hygieneChecked={hygieneChecked} onHygieneChange={handleHygieneChange} />
          </TabsContent>
        ))}
      </Tabs>
    </div>
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{medRecordsPhases.filter((p) => medRecordsTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
      <div className="mt-6 rounded-lg border border-border bg-card p-6 text-center">
        <p className="text-sm text-muted-foreground">Full KPI library, scorecard, and SLA tables available on the dedicated metrics pages.</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 overflow-auto">
      <LitifyMatterLayout
        title="Med Records &mdash; Matter Record"
        activeMatterId="medrec-matter"
        recordHeaderProps={{
          title: 'Martinez, Roberto | MVA Rear-End | 2024-0847',
          icon: ClipboardList,
          fields: [
            { label: 'Active Stage', value: stageProgress.find((s) => s.isActive)?.label ?? 'N/A' },
            { label: 'Providers', value: '6 identified' },
            { label: 'Records Status', value: '18/24 received' },
            { label: 'Owner', value: 'K. Patel' },
          ],
          statusBadge: {
        ...
        pathBarProps={{ stages: stageProgress, onStageClick: () => {}, currentTaskLabel: undefined }}
        tabs={[
