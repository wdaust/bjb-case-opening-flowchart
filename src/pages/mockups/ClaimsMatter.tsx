...
import {
  ...
  claimsTasks,
  CLAIMS_PATH_STAGES,
  getClaimsTasksForStage,
} from '../../data/claimsTaskData.ts';

const STORAGE_KEY = 'bjb-claims-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  claimsTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function ClaimsMatter() {
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(claimsPhases.map((p) => p.id)));

  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(taskStatuses)); } catch { /* ignore */ }
  }, [taskStatuses]);

  const completedCount = claimsTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = claimsTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(claimsTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  const stageProgress: PathBarStage[] = useMemo(() => {
    return CLAIMS_PATH_STAGES.map((stage) => {
      const tasks = getClaimsTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof claimsTasks>();
  ...
  const visiblePhases = claimsPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(claimsPhases.map((p) => p.id)));
  }, []);
  ...
  const overviewContent = (
    <OverviewTab tasks={claimsTasks} phases={claimsPhases} statuses={taskStatuses} scoringSystems={scoringSystems} scores={scores} />
  );
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {claimsPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = claimsTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{claimsPhases.filter((p) => claimsTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-auto">
      <LitifyMatterLayout
        title="Claims &mdash; Matter Record"
        activeMatterId="claims-matter"
        recordHeaderProps={{
          title: 'Martinez, Roberto | MVA Rear-End | 2024-0847',
          icon: ClipboardList,
          fields: [
            { label: 'Active Stage', value: stageProgress.find((s) => s.isActive)?.label ?? 'N/A' },
            { label: 'Demand Amount', value: '$125,000' },
            { label: 'Policy Limits', value: '$250K/$500K' },
            { label: 'Owner', value: 'M. Thompson' },
          ],
          statusBadge: {
