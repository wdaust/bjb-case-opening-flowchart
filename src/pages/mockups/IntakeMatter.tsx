...
import {
  ...
  intakeTasks,
  INTAKE_PATH_STAGES,
  getIntakeTasksForStage,
} from '../../data/intakeTaskData.ts';

const STORAGE_KEY = 'bjb-intake-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  intakeTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function IntakeMatter() {
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(intakePhases.map((p) => p.id)));

  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(taskStatuses)); } catch { /* ignore */ }
  }, [taskStatuses]);

  const completedCount = intakeTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = intakeTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(intakeTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  const stageProgress: PathBarStage[] = useMemo(() => {
    return INTAKE_PATH_STAGES.map((stage) => {
      const tasks = getIntakeTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof intakeTasks>();
  ...
  const visiblePhases = intakePhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(intakePhases.map((p) => p.id)));
  }, []);
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {intakePhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = intakeTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{intakePhases.filter((p) => intakeTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-auto">
      <LitifyMatterLayout
        title="Intake &mdash; Matter Record"
        activeMatterId="intake-matter"
        recordHeaderProps={{
          title: 'New Lead | MVA Intersection | 2024-1205',
          icon: ClipboardList,
          fields: [
            { label: 'Active Stage', value: stageProgress.find((s) => s.isActive)?.label ?? 'N/A' },
            { label: 'Lead Source', value: 'Web Form' },
            { label: 'Incident Date', value: '2024-12-01' },
            { label: 'Owner', value: 'J. Rivera' },
          ],
          statusBadge: {
