...
import {
  ...
  discTasks,
  DISC_PATH_STAGES,
  getDiscTasksForStage,
} from '../../data/discTaskData.ts';

// ── localStorage ──────────────────────────────────────────────────────

const STORAGE_KEY = 'bjb-disc-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  discTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function DiscMatter() {
  // ── State ──────────────────────────────────────────────────────────
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(discPhases.map((p) => p.id)));
  ...
  // ── Derived ────────────────────────────────────────────────────────
  const completedCount = discTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = discTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(discTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  // Path bar stages
  const stageProgress: PathBarStage[] = useMemo(() => {
    return DISC_PATH_STAGES.map((stage) => {
      const tasks = getDiscTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof discTasks>();
  ...
  const visiblePhases = discPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(discPhases.map((p) => p.id)));
  }, []);
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {discPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = discTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="rounded-lg border border-border bg-card p-6 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          <a href="/performance-infrastructure/mockups/disc-contact" className="text-blue-600 hover:underline">Discovery Contact Pursuit</a> page.
        </p>
      </div>
    </div>
  );
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{discPhases.filter((p) => discTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-auto">
      <LitifyMatterLayout
        title="Lit / Discovery &mdash; Matter Record"
        group="discovery"
        activeNavId="disc-matter"
        recordHeaderProps={{
