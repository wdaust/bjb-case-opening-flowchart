...
export default function ExpScoring() {
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-3">Expert &amp; Deposition — Scoring Dashboard</h1>
        <MockupNav active="exp-scoring" group="expert-deposition" />
      </div>

      {/* Expert & Deposition info banner */}
      <div className="mb-6 rounded-lg border border-stone-700/30 bg-stone-700/5 p-4">
        <p className="text-sm text-foreground">
          <span className="font-semibold">Re-score or confirm "No Change"</span>{' '}
          <span className="text-muted-foreground">
            — After each expert communication, all 5 scoring systems must be re-scored or have "No Change" entered within 1 hour. Expert &amp; deposition context: scores inform expert selection strategy, deposition readiness, and settlement leverage.
          </span>
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              {noChangeFlags[sys.id] && (
                <p className="text-center text-xs text-stone-700 font-medium">No Change</p>
              )}
            </button>
          );
        })}
          ...
          <TabsContent key={sys.id} value={sys.id}>
            ...
            <div className="mt-4 flex items-center gap-3 rounded-lg border border-border bg-card p-4">
              <Button
                ...
                size="sm"
                className={noChangeFlags[activeTab] ? "bg-stone-700 hover:bg-stone-800 text-white" : ""}
