...
import {
  ...
  TM_PATH_STAGES,
  getTmTasksForStage,
} from '../../data/tmTaskData.ts';

// ── localStorage ──────────────────────────────────────────────────────

const STORAGE_KEY = 'bjb-tm-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  tmTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function TMMatter() {
  // ── State ──────────────────────────────────────────────────────────
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(tmPhases.map((p) => p.id)));
  ...
  // ── Derived ────────────────────────────────────────────────────────
  const completedCount = tmTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = tmTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(tmTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  // Path bar stages
  const stageProgress: PathBarStage[] = useMemo(() => {
    return TM_PATH_STAGES.map((stage) => {
      const tasks = getTmTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof tmTasks>();
  ...
  const visiblePhases = tmPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(tmPhases.map((p) => p.id)));
  }, []);
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {tmPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = tmTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{tmPhases.filter((p) => tmTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
