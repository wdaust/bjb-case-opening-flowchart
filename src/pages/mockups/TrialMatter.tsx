...
import {
  ...
  trialTasks,
  TRIAL_PATH_STAGES,
  getTrialTasksForStage,
} from '../../data/trialTaskData.ts';

// ── localStorage ──────────────────────────────────────────────────────

const STORAGE_KEY = 'bjb-trial-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  trialTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
const phaseBorderColors: Record<string, string> = {
  ...
  'violet-700': 'border-l-violet-700', 'sky-700': 'border-l-sky-700', 'fuchsia-700': 'border-l-fuchsia-700',
};
...
export default function TrialMatter() {
  // ── State ──────────────────────────────────────────────────────────
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(trialPhases.map((p) => p.id)));
  ...
  // ── Derived ────────────────────────────────────────────────────────
  const completedCount = trialTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = trialTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(trialTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  // Path bar stages
  const stageProgress: PathBarStage[] = useMemo(() => {
    return TRIAL_PATH_STAGES.map((stage) => {
      const tasks = getTrialTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof trialTasks>();
  ...
  const visiblePhases = trialPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(trialPhases.map((p) => p.id)));
  }, []);
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {trialPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = trialTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{trialPhases.filter((p) => trialTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-auto">
      <LitifyMatterLayout
        title="Lit / Trial &mdash; Matter Record"
        activeMatterId="trial-matter"
        recordHeaderProps={{
