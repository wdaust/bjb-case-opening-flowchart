...
import {
  ...
  EXP_PATH_STAGES,
  getExpTasksForStage,
} from '../../data/expTaskData.ts';

// ── localStorage ──────────────────────────────────────────────────────

const STORAGE_KEY = 'bjb-exp-matter-statuses';
...
function loadTrackerStatuses(): Record<string, TrackerStatus> {
  ...
  expTasks.forEach((t) => { init[t.id] = 'pending'; });
  return init;
}
...
export default function ExpMatter() {
  // ── State ──────────────────────────────────────────────────────────
  ...
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(() => new Set(expPhases.map((p) => p.id)));
  ...
  // ── Derived ────────────────────────────────────────────────────────
  const completedCount = expTasks.filter((t) => taskStatuses[t.id] === 'complete').length;
  const totalCount = expTasks.length;
  const progressPct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

  const uniqueAssignees = useMemo(() => {
    const set = new Set(expTasks.map((t) => t.assignedTo));
    return Array.from(set).sort();
  }, []);

  // Path bar stages
  const stageProgress: PathBarStage[] = useMemo(() => {
    return EXP_PATH_STAGES.map((stage) => {
      const tasks = getExpTasksForStage(stage);
  ...
  const tasksByPhase = useMemo(() => {
    const map = new Map<string, typeof expTasks>();
  ...
  const visiblePhases = expPhases.filter((p) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setFilterStatus('all');
    setExpandedPhases(new Set(expPhases.map((p) => p.id)));
  }, []);
    ...
    <div className="py-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterPhase} onChange={(e) => setFilterPhase(e.target.value)}
          ...
          {expPhases.map((p) => <option key={p.id} value={p.id}>{p.order}. {p.label}</option>)}
        </select>
      ...
      <div className="space-y-4">
        {visiblePhases.map((phase) => {
          ...
          const allPhaseTasks = expTasks.filter((t) => t.phase === phase.id);
    ...
    <div className="py-4">
      <div className="rounded-lg border border-border bg-card p-6 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          <a href="/performance-infrastructure/mockups/exp-contact" className="text-blue-600 hover:underline">Expert & Deposition Contact Pursuit</a> page.
        </p>
      </div>
    </div>
  );
    ...
    <div className="py-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{expPhases.filter((p) => expTasks.filter((t) => t.phase === p.id && taskStatuses[t.id] === 'in-progress').length > 0).length}</p>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 overflow-auto">
      <LitifyMatterLayout
        title="Lit / Expert &amp; Deposition &mdash; Matter Record"
        group="expert-deposition"
        activeNavId="exp-matter"
        recordHeaderProps={{
