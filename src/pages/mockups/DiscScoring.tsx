...
export default function DiscScoring() {
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-3">Discovery — Scoring Dashboard</h1>
        <MockupNav active="disc-scoring" group="discovery" />
      </div>

      {/* Discovery info banner */}
      <div className="mb-6 rounded-lg border border-blue-800/30 bg-blue-800/5 p-4">
        <p className="text-sm text-foreground">
          <span className="font-semibold">Re-score or confirm "No Change"</span>{' '}
          <span className="text-muted-foreground">
            — After each client communication, all 5 scoring systems must be re-scored or have "No Change" entered within 1 hour. Discovery context: scores inform deposition readiness and discovery response strategy.
          </span>
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              {noChangeFlags[sys.id] && (
                <p className="text-center text-xs text-blue-700 font-medium">No Change</p>
              )}
            </button>
          );
        })}
          ...
          <TabsContent key={sys.id} value={sys.id}>
            ...
            <div className="mt-4 flex items-center gap-3 rounded-lg border border-border bg-card p-4">
              <Button
                ...
                size="sm"
                className={noChangeFlags[activeTab] ? "bg-blue-800 hover:bg-blue-900 text-white" : ""}
