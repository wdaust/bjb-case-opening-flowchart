...
import { expertDepoProtocolAgenda } from '../../data/expAppointmentData.ts';
...
export default function ExpAppointment() {
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set([expertDepoProtocolAgenda[0]?.id]),
  );
  ...
  // Derived progress
  const completedSections = expertDepoProtocolAgenda.filter((section) =>
    isSectionComplete(section.id, section.checkItems, checkedItems),
  ).length;

  const totalSections = expertDepoProtocolAgenda.length;
    ...
    <div className="flex-1 overflow-auto p-6">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-3">
          Expert &amp; Deposition — Retention Protocol
        </h1>
        <MockupNav active="exp-appointment" group="expert-deposition" />
      </div>
      ...
      <div className="mb-6 rounded-lg bg-gradient-to-r from-stone-700 to-stone-800 p-4 text-white">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
          <div>
            <span className="text-xs text-stone-300">Case #</span>
            <p className="font-mono font-bold">2024-1547</p>
          </div>
          <div>
            <span className="text-xs text-stone-300">Client</span>
            <p className="font-semibold">Ramirez, Ana &mdash; MVA Highway</p>
          </div>
          <div>
            <span className="text-xs text-stone-300">Stage</span>
            <p className="text-sm">Expert &amp; Deposition</p>
          </div>
          <div>
      ...
      <div>
        <div className="grid gap-6 lg:grid-cols-4">
          ...
          <div className="space-y-4 lg:col-span-3">
            {expertDepoProtocolAgenda.map((section) => {
                  ...
                  >
                    ...
                    <Badge
                      ...
                      className={cn(
                        'shrink-0 text-xs',
                        complete
                          ? 'border-stone-600/40 bg-stone-600/10 text-stone-700'
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {section.litifyFields.map((field, idx) => (
                              <Badge
                                key={idx}
                                variant="outline"
                                className="border-stone-600/30 bg-stone-600/5 text-xs text-stone-600"
                              >
              ...
              <div className="rounded-lg border border-border bg-card p-5 shadow-sm">
                ...
                <div className="mb-3 flex items-center justify-between">
                  <span className="text-sm text-foreground">
                    <span className="text-lg font-bold text-stone-700">{completedSections}</span>
                    {' '}of {totalSections} sections complete
                  </span>
                  <CheckCircle2
                    className={cn(
                      'h-5 w-5',
                      allComplete ? 'text-stone-700' : 'text-muted-foreground/40',
                    )}
                  />
                </div>
                ...
                <div className="mb-6 h-2.5 w-full overflow-hidden rounded-full bg-muted">
                  <div
                    className={cn(
                      ...
                      allComplete ? 'bg-stone-600' : 'bg-stone-700',
                    )}
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          done ? 'text-stone-700' : 'text-muted-foreground',
                        )}
                      >
                        <span
                          className={cn(
                        ...
                        {done && <CheckCircle2 className="ml-auto h-3.5 w-3.5 shrink-0 text-stone-700" />}
                      </button>
                    );
                  })}
                </div>
