...
export default function ArbMedScoring() {
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-3">Arbitration/Mediation — Scoring Dashboard</h1>
        <MockupNav active="arbmed-scoring" group="arbitration-mediation" />
      </div>

      {/* Arb/Med info banner */}
      <div className="mb-6 rounded-lg border border-amber-700/30 bg-amber-700/5 p-4">
        <p className="text-sm text-foreground">
          <span className="font-semibold">Re-score or confirm &quot;No Change&quot;</span>{' '}
          <span className="text-muted-foreground">
            — After each arb/med communication, all 5 scoring systems must be re-scored or have &quot;No Change&quot; entered within 1 hour. Arbitration/mediation context: scores inform settlement strategy, mediation positioning, and De Novo decision-making.
          </span>
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              {noChangeFlags[sys.id] && (
                <p className="text-center text-xs text-amber-700 font-medium">No Change</p>
              )}
            </button>
          );
        })}
          ...
          <TabsContent key={sys.id} value={sys.id}>
            ...
            <div className="mt-4 flex items-center gap-3 rounded-lg border border-border bg-card p-4">
              <Button
                ...
                size="sm"
                className={noChangeFlags[activeTab] ? "bg-amber-700 hover:bg-amber-800 text-white" : ""}
